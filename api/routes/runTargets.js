...
exports.targetLites = function(req, res, session) {
    res.setHeader('Content-Type', 'application/json');
    new RunTargets()
        ...
        .exec(function(error, runTargets) {
            if(error) {
                ...
                var targets = runTargets.models.map( function( runTarget ) {
                    var percentComplete = runTarget.relations.runEvents.models.reduce(function(prev, curr) {
                        return prev += curr.attributes.distance;
                    }, 0);

                    var target = {
                        name: runTarget.attributes.name,
                    id: runTarget.attributes.id,
                    percentComplete: percentComplete / runTarget.attributes.total_miles
                    };
